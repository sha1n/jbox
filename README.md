...
- **Not a virtual audio driver.** Jbox does not ship its own audio device. The virtual-output feature (Phase 7.6) builds on **[BlackHole](https://github.com/ExistentialAudio/BlackHole)** — users install BlackHole separately, and Jbox then recognises it as a virtual endpoint so media apps can route audio *through* Jbox to a real destination. See [docs/spec.md § 2.13](./docs/spec.md#213-virtual-output-via-blackhole).
...
Phase 7.6 virtual-output slice is **next up**, promoted from the post-v1.0.0 deferred list because the two-interface live-monitoring scenario Phase 7.5 was supposed to unlock runs into a macOS hard constraint: shared real devices run at `max(client buffers)`, so a snappy route sharing its destination with media apps inherits the apps' large buffer. The original plan was an in-house `AudioServerPlugIn`; after manual testing confirmed that ad-hoc-signed HAL plugins do not load under macOS 13+ coreaudiod's out-of-process sandbox, the phase was rebuilt on top of [**BlackHole**](https://github.com/ExistentialAudio/BlackHole) — users install BlackHole separately (free, open source, Developer-ID signed + notarized, `brew install blackhole-2ch` or a one-click `.pkg`), and Jbox recognises BlackHole devices as virtual endpoints that skip the hog-mode claim and collapse the drift loop to rate-identity. ABI v9 → v10 (`is_virtual` byte). The in-house-driver prototype is preserved on branch `archive/phase7.6-own-driver` as institutional memory. See [docs/plan.md Phase 7.6](./docs/plan.md#phase-76--virtual-output-via-blackhole) and [docs/spec.md § 2.13](./docs/spec.md#213-virtual-output-via-blackhole).
