...
Still pending in Phase 6: Preferences window beyond the diagnostics toggle, VoiceOver label on the expanded meter panel, and `XCUITest` flows. The scene editor was consolidated into Phase 7 alongside persistence. Real-hardware acceptance tests (soak, latency, sample-rate mismatch, multi-route sanity) from Phases 3–5 are deferred to the owner's rig — the simulated backend covers their logic.
