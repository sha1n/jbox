...
- **dB-scale-to-bar-zone alignment fix (issue #12).** The original mixer-strip cut shipped `SectionScale` with `.padding(.vertical, capBandHeight)` (10 pt top + bottom) while the bar zone in `MixerStripColumn` is offset by 40 pt at the top (header + +12 cap + outer pad + spacings) and 66 pt at the bottom (action + readout + −∞ cap + outer pad + spacings) — so the "0 / -3 / … / -60" ticks did not line up with the bar fill or the peak-hold tick. Fix: extracted `bandStackInnerSpacing` (4 pt) and `bandStackOuterPadding` (6 pt) into `MixerPanelLayout`, derived `barZoneTopOffset` (= 40) and `barZoneBottomOffset` (= 66) from those + the existing band heights, switched `SectionScale` to `.padding(.top: barZoneTopOffset, .bottom: barZoneBottomOffset)`, and re-routed `MixerStripColumn`'s VStack spacing + outer padding through the same constants so a future band-height edit cascades into the offsets automatically. `MixerLayoutTests` now pin the offset formulas + concrete values (40 / 66) so a band-stack composition change can't silently re-introduce the misalignment.
...
- [x] **Stress / disconnect test.** Start and stop routes rapidly. Unplug and replug devices. Verify graceful recovery, no crashes, no stuck routes. Document procedure in `docs/testing/stress.md`. *Procedure drafted in [`docs/testing/stress.md`](./testing/stress.md); first hardware acceptance pass completed 2026-05-12 against the `v0.1.0-alpha` DMG — all three F1 criteria green. Run record at [`docs/2026-05-12-f1-hardware-acceptance-debug-session.md`](./2026-05-12-f1-hardware-acceptance-debug-session.md). Rapid start/stop matrix is the remaining half of this task and stays deferred until that procedure lands in `stress.md`.*
...
- **F1 hardware acceptance pass completed (2026-05-12).** First end-to-end hardware acceptance run of the stress / disconnect procedure landed against the published `v0.1.0-alpha` DMG, two-device rig (USB source + Thunderbolt destination + an aggregate wrapping both). All three F1 acceptance criteria green — hot-unplug a sub-device, yank the source USB interface, sample-rate cascade in Audio MIDI Setup. F3 (event debounce) stays optional, not promoted to blocker. A side-anomaly on the post-replug latency pill was investigated through three hypotheses (stale IOProc retention, in-session engine state corruption, polluted `coreaudiod` daemon state) and ruled out as an engine bug; the residual gap is a documentation issue (direct-cross-device path latency dominated by the destination's external I/O buffer setting) and not in scope for the engine. F1 in `docs/followups.md` is closed. Full run record + investigation chronology at [`docs/2026-05-12-f1-hardware-acceptance-debug-session.md`](./2026-05-12-f1-hardware-acceptance-debug-session.md). Phase 9's soak and latency procedures remain open; the stress / disconnect half of F6 is now closed.
