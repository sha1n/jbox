...
| 7.6   | Self-routing reliability              | Drop hog mode + HAL buffer-shrink. Users dial buffer in their interface software; Jbox respects it. Plus a per-route SD-style buffer preference (no hog), device hot-plug, sleep/wake, error-state UX. | 🚧 Big simplification landed (cuts ~2400 lines + Phase 7.5's ABI/UI surface). Re-added a no-hog `setBufferFrameSize` (ABI v10 → v11 additive) after Superior Drummer demonstrated the cascade was hog-eviction-side, not property-write-side. **7.6.3 robust teardown landed; 7.6.4 engine-side hot-plug listeners + auto-recovery landed (ABI v11 → v12 additive with `JBOX_ERR_DEVICE_GONE`).** 7.6.4's CoreAudioBackend HAL property listener wiring deferred to a hardware-tested follow-up. 7.6.5 still pending. |
