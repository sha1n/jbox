...
| 7     | Persistence + launch-at-login         | Relaunching the app restores configured routes; opt-in launch-at-login.                | 🚧 Persistence slice landed — routes + preferences round-trip through `state.json`. Launch-at-login still pending. Scenes (and the sidebar shell that hosted them) **deferred to a future release** — see § 4.10 in `docs/spec.md` and the "After v1.0.0 — deferred work" entry below; v1 carries no schema reservation, the feature returns as a `v1 → v2` migration. |
...
**Status:** 🚧 Persistence slice landed (Codable layer + `StateStore` + `AppState` wiring + live route / preferences round-trip). Launch-at-login is still pending. Scenes — and the sidebar shell that hosted them — were **deferred to a future release**; see the deviation note + the "After v1.0.0 — deferred work" entry. No scene scaffolding survives in v1; when the feature returns it lands as a single slice including a `v1 → v2` schema migration.
