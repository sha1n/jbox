...
Buffer-size handshake: because the fast path skips the mux, it issues its own buffer-frame-size request — snapshots `currentBufferFrameSize` on start, calls `requestBufferFrameSize(uid, 64)` when the pre-change value is larger than the target, then re-reads the post-change value and folds it into `LatencyComponents::src_buffer_frames` so the pill shows what the HAL actually honored (Core Audio's max-across-clients policy can keep the buffer larger when another app is holding the device). `releaseRouteResources` restores the snapshot on stop.
Exclusive device ownership: the fast path also attempts `IDeviceBackend::claimExclusive` before issuing the buffer request. That wraps Core Audio's `kAudioDevicePropertyHogMode` — setting it to the process's PID disconnects every other client of the device (UAD Console, DAWs, system audio) for the duration and lets the buffer request land at the requested size instead of being clamped by another app's larger preference. On stop the buffer size is restored first, then hog mode is released in that order so external apps re-read the original buffer when they reconnect. Claim failure is non-fatal — the route continues on the shared path and accepts whatever buffer size the HAL ends up giving. The UI copy for the Performance tier documents the exclusive-ownership behavior explicitly.
...
**Device buffer-frame-size refcounting.** `DeviceIOMux` also tracks how many attached routes opted into `low_latency`. On the 0 → 1 transition it snapshots the device's current `kAudioDevicePropertyBufferFrameSize` and asks the backend (`IDeviceBackend::requestBufferFrameSize`) to shrink it to the low-latency target (64 frames — clamped into the HAL-reported range on `CoreAudioBackend`). On the 1 → 0 transition (last low-latency route detaching) it asks the backend to restore the snapshot. The counter spans both input and output directions because `kAudioDevicePropertyBufferFrameSize` is a global device property. Non-low-latency routes neither bump nor decrement the counter; they coexist with a shrunk buffer but never themselves trigger a change. Note: the HAL property is visible to every process — other applications using the same device will see the buffer change for the duration of the low-latency route's lifetime. This is documented in the UI copy for the "Low latency" toggle and is the explicit trade-off.
