...
- **Stall watchdog.** A 500 ms freeze of both `frames_produced` and `frames_consumed` on a `RUNNING` route triggers `WAITING + JBOX_ERR_DEVICE_STALLED` (ABI v15). Independent safety net catching "silent death" cases the HAL listeners don't surface (another app preempts the device, an aggregate IOProc freezes without an `IsAlive=0`, an undocumented Apple edge). Driven by `RouteManager::tickStallWatchdog(now)` on the engine's existing 10 Hz hot-plug consumer; per-route counters are re-primed on every entry into `RUNNING` so a fresh start clears the snapshot.
- **Periodic WAITING-route retry.** Independently of the HAL-event-driven recovery in `handleDeviceChanges`, the engine's hot-plug consumer calls `RouteManager::retryWaitingRoutes()` at 1 Hz. The pass refreshes the device manager and calls `attemptStart` on every `WAITING` route. Generalises recovery so a route comes back automatically when its devices return — even when the HAL fires no `kDeviceListChanged` / `kAggregateMembersChanged` event (Apple's HAL is unreliable for "device powered off and back on" because the audio object often persists; only the sample flow stops and resumes). `attemptStart` fast-fails on still-missing devices, so the steady-state cost is one `enumerate()` per second when at least one route is `WAITING`.
- **Recovery-path error classification (sub-phase 7.6.7).** Device-side conditions encountered during a recovery `attemptStart` (channel mismatch when the parent aggregate's channel count has shrunk, mux-attach refusal, `openDuplexCallback` failure, residual duplex IOProc handle from a prior failed teardown) park the route in `WAITING` (with `JBOX_ERR_DEVICE_GONE` for channel mismatch or `JBOX_ERR_DEVICE_BUSY` for mux/duplex transients) so the periodic retry / next event recovers the route automatically. `ERROR` is reserved for genuine config errors and programming/allocation failures: a route whose mapping references a channel that doesn't exist on a fully-present device, an `AudioConverterWrapper` allocation that raises. The `RouteRecord::ever_started` flag splits the channel-mismatch ambiguity — never-started routes go to `ERROR + JBOX_ERR_MAPPING_INVALID` (the user has misconfigured); previously-running routes go to `WAITING + JBOX_ERR_DEVICE_GONE` (the device is degraded but coming back is plausible).
- **Targeted per-device HAL listeners (sub-phase 7.6.7).** `IDeviceBackend::setWatchedUids(std::vector<std::string>)` is the host's signal to the backend: only these UIDs need per-device listeners (`kAudioDevicePropertyDeviceIsAlive`, `kAudioAggregateDevicePropertyActiveSubDeviceList`). `RouteManager::publishWatchedUids()` computes the union of `{source_uid, dest_uid}` ∪ aggregate members across every route NOT in `STOPPED` or `ERROR` and pushes it to the backend after every state-changing entry point. `CoreAudioBackend::reconcilePerDeviceListeners` filters its install/remove diff against the published set, so listener load tracks the routing set rather than the entire system inventory. The system-wide `kAudioHardwarePropertyDevices` listener stays unconditionally registered — it is what surfaces a previously-unwatched UID showing up.
- **Sleep/wake.** `IORegisterForSystemPower` is the source of truth: `kIOMessageSystemWillSleep` stops every running route and replies `IOAllowPowerChange`; `kIOMessageSystemHasPoweredOn` refreshes enumeration and best-effort restarts the snapshot of pre-sleep routes with bounded retries (3 attempts, linear backoff). `last_error = JBOX_ERR_SYSTEM_SUSPENDED` (ABI v13) so the UI can distinguish sleep-recovery from device-loss.
...
The expanded route panel uses a **mixer-strip layout** (introduced with the per-route gain feature) split into two outlined sections — **SOURCE** (input meters) on the left and **DESTINATION** (per-channel strips + VCA on the far right) on the right. Each section is a `MeterSectionFrame` with a centered title above its content and a thin rounded border around the whole thing.

**Section widths** are partitioned so the two sections have an identical per-channel-strip area — DESTINATION is wider only by exactly one VCA-strip slot (`MixerPanelLayout.vcaSlotWidth` = VCA strip width + strip spacing). The math lives in `MixerPanelLayout.sectionWidths(panelInnerWidth:isCompact:)` and is unit-tested (`MixerLayoutTests`). Both sections clamp at `minSectionWidth` (120 px) on a too-narrow window and rely on horizontal scroll inside each section for overflow.

**Strip composition** — channel and source-meter strips are the same `MixerStripColumn` widget under different `MixerStripDimensions.Style` cases; their outer width and meter width match. The VCA strip is wider, with a heavier border. Every strip uses the same band stack (header / +12 cap / bar zone / −∞ cap / readout / mute) at fixed band heights so bar zones align across the panel HStack.

**Mirror layout** — SOURCE meters render in the slot where the DESTINATION strip's *fader* sits (leading position) — the destination's meter slot becomes a phantom spacer on source strips. Source meters therefore sit on the left of each strip and destination meters on the right, like two halves of a mixer facing each other across the panel center. Each section pins its own dBFS scale column (`DbScale`, marks from `MeterLevel.dawScaleMarks`: `0, -3, -6, -12, -18, -24, -36, -48, -60`) on the left of its bars; the VCA strip pins to the far right of the destination section. The dB-scale column's canvas is padded by `MixerPanelLayout.barZoneTopOffset` / `barZoneBottomOffset` (sums of the bands above/below the bar zone in `MixerStripColumn`) so its `0…-60` ticks span exactly the same y-range as the bar fill — `MixerLayoutTests` pin both offsets to the strip column's actual band-stack composition so a future band-height change can't drift the alignment again.
