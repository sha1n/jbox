...
**Buffer size: user-managed.** Phase 7.6 removed Jbox's HAL hog-mode and buffer-frame-size negotiation. The user dials their interface buffer in their interface software once (UA Console / RME TotalMix / MOTU CueMix / Audio MIDI Setup); Jbox respects whatever it finds, and the latency pill reflects the honest end-to-end estimate. Anyone running a DAW already manages their interface buffer the same way.
